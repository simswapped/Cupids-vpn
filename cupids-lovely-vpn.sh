...
echo -e "\n\nThanks for installing Cupids Lovely VPN. The whole process will take a few minutes depending on your system specs.\n"
...
echo -e "2. Creating Lovely config file and relevant dirs"


{

mkdir /etc/zenected
echo -e "#Lovely config\n" > /etc/zenected/zenected.conf

 CURRENT_IP=`curl -k -s https://zenected.com/remote_ip.php`
 echo -e "LUV_IP=\""$CURRENT_IP"\"" >> /etc/zenected/zenected.conf
...
echo -e LUV_RADIUS_SHARED=\""$RADIUS_SHAREDPASS"\"" >> /etc/zenected/zenected.conf
sleep 2
...
#THE END
echo -e "\n\nThanks for your patience. Cupids Lovely VPN install script has now finished.\nRemember to check Resources (manuals and tips) at https://zenected.com\n\nYou can now continue to configure Zenected.\n"
read -r -n 1 -s -p "Press any key to continue and run Cupids Lovely VPN Setup..."
